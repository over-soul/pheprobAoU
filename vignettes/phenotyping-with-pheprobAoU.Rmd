---
title: "Phenotyping with pheprobAoU: A Comprehensive Guide"
...
output: 
  rmarkdown::html_vignette:
    ...
    toc_depth: 3
vignette: >
  %\VignetteIndexEntry{Phenotyping with pheprobAoU: A Comprehensive Guide}
...
The `pheprobAoU` package provides a comprehensive toolkit for calculating PheProb (phenotype probabilities) using electronic health record (EHR) data from the All of Us Research Program. This vignette demonstrates real-world applications of the package for common phenotyping tasks.

## What is PheProb?

PheProb represents the likelihood that a participant exhibits a particular phenotype based on their clinical history recorded in EHR data. Unlike simple binary phenotype definitions, PheProb provides a probabilistic approach that:

- Accounts for varying levels of clinical evidence
- Incorporates temporal patterns in medical history
- Handles missing or incomplete data gracefully
- Provides graded phenotype assessments rather than binary classifications
...
The first step in any phenotyping analysis is to identify the relevant OMOP concept IDs that define your phenotype of interest. 
...
## Step 2: Calculate PheProb Scores

```{r}
# Basic PheProb calculation
...
hist(diabetes_scores$pheprob_score, 
     main = "Distribution of Diabetes PheProb Scores",
     xlab = "PheProb Score",
     ylab = "Frequency")

# Identify high-probability cases
high_prob_diabetes <- diabetes_scores[diabetes_scores$pheprob_score > 0.8, ]
cat("Number of high-probability diabetes cases:", nrow(high_prob_diabetes))
```

# Advanced Phenotyping Examples

## Multiple Phenotypes Analysis

One of the key improvements in pheprobAoU is the ability to analyze multiple unrelated phenotypes simultaneously, avoiding the meaningless aggregation that occurs when mixing unrelated concept IDs.

### The Problem with Mixed Concepts

```{r}
# DON'T DO THIS - mixing unrelated diseases
mixed_concepts <- c(
...
# The resulting pheprob_score would be meaningless
# It's not the probability of any coherent phenotype!
```

### Solution: Multiple Phenotypes Mode

```{r}
# BETTER APPROACH - define separate phenotypes
phenotypes <- list(
...
# View results - each phenotype gets its own meaningful probability
head(multi_scores)
```
